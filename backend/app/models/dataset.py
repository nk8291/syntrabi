...
class Dataset(Base):
    ...
    connector_type = Column(
        SQLEnum(ConnectorType, name='connector_type', create_constraint=False, values_callable=lambda x: [e.value for e in x]),
        nullable=False
    )
    ...
    # Processing status
    status = Column(
        SQLEnum(DatasetStatus, name='dataset_status', create_constraint=False, values_callable=lambda x: [e.value for e in x]),
